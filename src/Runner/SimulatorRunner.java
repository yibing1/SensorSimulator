 ...
 * SimulatorRunner 类用于模拟传感器工作模式，包括传感器的发送采集数据，接受指令，
   * 停止仪器。该程序主要工作模式为读取指定文件（数据），连续循环向指定的串口发送数据，同时
   * 在接收到仪器命令时，做出相应的工作。
 * 
 * @author Yibing Zhang
 */
public class SimulatorRunner
{
     ...
     * @param args args[0] 串口号 args[1] 模拟器名称 args[2] 数据文件名称  args[3] 波特率
     *  args[4] 文件类别：1为非2进制文件 2 为2进制文件
     */
        ...
        public void run()
        {
            while (true)
            {
                while (queue.size() > 0)
                {
                    ...
                    //设置handler的参数
                    handler.iniHandler(task, port, fileName, fileType);
                    //对命令做出反应
                    task = handler.doWork();
                }
            }
